            ...
            try 
            {
                ...
                while($row = $query->fetch(PDO::FETCH_ASSOC))
                {
                    ...
                    "<div class='center parent'>" . 
                        "<a href='../views/producto_hecho/producto.php?id=" .$producto->getId() . "'>" .
...
if ($_SERVER["REQUEST_METHOD"]=="POST") 
{
    if($_POST["_method"]=="POST")
    {
        ...
        try
        {
            $query = $connection->prepare('INSERT INTO producto VALUES(NULL,:tipo, 
            ...
            header("Location:http://localhost/Proyecto Web (Rappel)/views/añadir_producto/añadir_producto.php?exito=Producto añadido con exito");
        }
        catch (PDOException $e) 
        {
            var_dump($e);
    ...
    else if($_POST["_method"]=="PUT")
    {
        ...
        try 
        {
            ...
            $query = $connection->prepare('UPDATE producto SET tipo=:tipo, 
            descripcion=:descripcion, precio=:precio, color=:color, talla=:talla,
            foto_tipo=:foto_tipo, foto_diseno=:foto_diseno WHERE id=:id');
            $query->bindParam(':id',$id,PDO::PARAM_INT);
            $query->bindParam(':tipo',$tipo,PDO::PARAM_STR);
            $query->bindParam(':descripcion',$descripcion,PDO::PARAM_STR);
            $query->bindParam(':precio',$precio,PDO::PARAM_INT);
    ...
    else if($_POST["_method"]=="DELETE")
    {
        ...
        try 
        {
            $query=$connection->prepare('DELETE FROM producto WHERE id=:id');
            $query->bindParam(':id',$id,PDO::PARAM_INT);
            $query->execute();
